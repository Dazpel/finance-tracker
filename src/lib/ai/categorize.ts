...
// Static system prompt for the AI fallback tier. Hoisted to module scope (and
// exported) so tests can assert its contents without triggering an LLM call —
// the category rules here are the only place category behavior lives, so a
// rename in @lib/categories that isn't reflected here would otherwise regress
// silently. Only depends on CANONICAL_CATEGORIES; no per-request state.
export const SYSTEM_PROMPT = [
  ...
  "",
  "   Charity (donations to charitable / non-profit organizations):",
  "   - animal welfare (Humane Society, ASPCA, animal rescue/shelter, pet adoption), religious orgs/churches, food banks, disaster relief (Red Cross), non-profit foundations, and donations/crowdfunding for a cause (e.g. GoFundMe for charity) => Charity",
  "",
  ...
  "- Charity is donations to registered charities / non-profits / religious orgs. Do NOT classify as Charity: political contributions, club/membership dues that return a personal benefit, or purchases from a store that happens to be non-profit-run — those follow their normal category.",
  "",
