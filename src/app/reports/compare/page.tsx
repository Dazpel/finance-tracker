...
const styledBreakdown = (breakdown: string, index: number) => {
  ...
  const isRevenueOrTotal =
    breakdown.includes("Revenue") || breakdown.includes("Total");
  ...
  const splitText = isIncreased
    ? breakdown.split("increased")
    : breakdown.split("decreased");
    ...
    Object.entries(report1).map(([key, value]) => {
      ...
      if (difference !== 0) {
        ...
        comparisonResults.push(
          `${formattedKey} ${direction} by $${Math.abs(difference).toFixed(2)}`
        );
      }
    });
